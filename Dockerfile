...
ENV BISERVER_RELEASE=9.1.0.0 \
	BISERVER_BUILD=324 \
	BISERVER_HOME=/pentaho-server \
	ECLIPSE_SWT_VERSION=4.6 \
	SYSLOG4J_VERSION=0.9.46 \
	KARAF_ASSEMBLY_VER=8.3.0.23-1230
...
RUN apt-get update \
	...
	&& wget --timeout=10 --waitretry=2 --tries=50 --retry-connrefused --progress=dot:giga https://public.nexus.pentaho.org/content/groups/omni/pentaho/pentaho-karaf-assembly/$KARAF_ASSEMBLY_VER/pentaho-karaf-assembly-$KARAF_ASSEMBLY_VER-client.zip \
	...
	&& mvn install:install-file -Dfile=pentaho-karaf-assembly-$KARAF_ASSEMBLY_VER-client.zip -DgroupId=pentaho -DartifactId=pentaho-karaf-assembly -Dversion=$KARAF_ASSEMBLY_VER -Dpackaging=zip \
	...
	&& sed -i -e 's|\(/packaging>\)|\1\n  <classifier>client</classifier>|' /root/.m2/repository/pentaho/pentaho-karaf-assembly/$KARAF_ASSEMBLY_VER/pentaho-karaf-assembly-$KARAF_ASSEMBLY_VER.pom \
